...
# Require statement will cause online tests to fail or error - I think! (they're passing locally)

Letter = Struct.new(:letter) do
  def tile_letter
    # I18n.transliterat converts characters like é to e, etc.
    # - requires active_support/inflector
    # - assumes tile set does not have diacritic marks
    I18n.transliterate(letter).upcase.to_sym
  end
end
...
class Scrabble
  ...
  def build_tile_set(tile_set)
    tile_set.each_with_object({}) { |(letter, points), set| set[letter] = points }
    tile_set.default_proc = proc { 0 }
    tile_set
  end
...
if $PROGRAM_NAME == __FILE__
  ...
  puts Scrabble.new('soirée').score # 6   # Scoring é as e
end
