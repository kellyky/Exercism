class TwelveDays
  ...
  def generate_song #(day_of_xmas = 1)
    1.upto(12) { |day_of_xmas| @song += verse(day_of_xmas)}
  ...
  def verse(day_of_xmas)
    ...
    "#{verse_intro(day_of_xmas) + gifts.join(', ')}." + newlines(day_of_xmas)
  end

  def build_verse_gifts(day_of_xmas)
    gifts = []
    day_of_xmas.downto(1).each { |day_counter| gifts << gift(day_of_xmas, day_counter) }
    gifts
  end

  def newlines(day_of_xmas)
    day_of_xmas == 12 ? "\n" : "\n\n"
  end
