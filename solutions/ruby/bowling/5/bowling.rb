...
class Score < Game
  ...
  def spare?
    throws.sum == PINS_PER_FRAME && throws.size == 2
  end

  def open?
    throws.sum < PINS_PER_FRAME
  end
end
...
class Frame < Game
  ...
  def spare?(rolls)
    rolls.sum == PINS_PER_FRAME
  end

  def build(roll)
    raise Game::BowlingError, 'Too many pins' if too_many_pins?(roll)
  ...
  def earlier_frame_full?(rolls)
    rolls.first == STRIKE || rolls.size == MAX_ROLLS_PER_REGULAR_FRAME
  end

  def more_than_ten_pins?(roll)
    frames[@frame_number].sum + roll > PINS_PER_FRAME
  end

  def too_many_pins?(roll)
    return true if roll > PINS_PER_FRAME

    tenth_frame? ? too_many_pins_tenth_frame?(roll) : more_than_ten_pins?(roll)
  end
  ...
  def too_many_pins_tenth_frame?(roll)
    ...
    tenth_frame.size == 2 && tenth_frame.first == STRIKE && tenth_frame[1] + roll > PINS_PER_FRAME
  end
end
