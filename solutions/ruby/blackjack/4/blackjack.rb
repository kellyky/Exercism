class Blackjack
  ...
  def play
    case
    when @player_hand.two_aces?
      'P'
    when @player_hand.blackjack? && !@dealer_card.ten_or_higher?
      'W'
    when player_blackjack_and_dealer_high? || @player_hand.high_range_score? || player_mid_and_dealer_low?
      'S'
    else 
      'H'
    end
